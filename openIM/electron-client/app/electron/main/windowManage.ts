...
class WindowManager {
  ...
  private attachMainWindowEvents(targetWindow: BrowserWindow) {
    ...
    targetWindow.on("close", (e) => {
      const closeAction = (this.store.get("closeAction") as string) || "quit";
      const closeActionWarned = Boolean(this.store.get("closeActionWarned"));
      let forceQuit = Boolean(global.forceQuit);
      const shouldWarnQuit =
        closeAction === "quit" &&
        !closeActionWarned &&
        !forceQuit &&
      ...
      if (shouldWarnQuit) {
        const response = dialog.showMessageBoxSync(targetWindow, {
          type: "warning",
          title: t("placeholder.exitApplication"),
          message: t("toast.closeWindowExitTip"),
          buttons: [
            t("placeholder.exitApplication"),
            t("placeholder.adjustCloseAction"),
            t("cancel"),
          ],
          defaultId: 0,
          cancelId: 2,
        });
        this.store.set("closeActionWarned", true);
        if (response === 1) {
          e.preventDefault();
          this.sendEvent(IpcMainToRender.eventTransfer, {
            event: "OPEN_PERSONAL_SETTINGS",
            args: {
              target: "main",
              highlightCloseAction: true,
            },
          });
          return;
        }
      ...
      if (closeAction === "quit" || forceQuit || !targetWindow.isVisible()) {
