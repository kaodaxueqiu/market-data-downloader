...
export function useConversationSettings() {
  ...
  const updateConversationMessageRemind = useCallback(
    async (checked: boolean, option: MessageReceiveOptType) => {
      ...
      try {
        ...
        feedbackToast({
          error,
          msg: t("system.toast.setConversationRecvMessageOptFailed"),
        });
      }
    },
    [currentConversation?.conversationID],
  ...
  const updateConversationPrivateState = useCallback(async () => {
    ...
    try {
      ...
      feedbackToast({
        error,
        msg: t("system.toast.updateConversationPrivateStateFailed"),
      });
    }
    ...
    modal.confirm({
      ...
      onOk: async () => {
        try {
          ...
          feedbackToast({
            error,
            msg: t("system.toast.clearConversationMessagesFailed"),
          });
        }
      },
    });
