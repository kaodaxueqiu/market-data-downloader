...
import { momentsQueryKey, useUserMoments } from "@/api/hooks/moments";
import { getMomentsByID } from "@/api/services/moments";
...
export const MomentsContent = ({
    ...
    const customMessageHandler = ({
      ...
      if (key.includes("wm_")) {
        ...
        queryClient.setQueryData<InfiniteData<WorkMoments[]> | undefined>(
          momentsQueryKey(routeTravel?.userID),
          (oldData) => {
  ...
  workMoments.forEach((moment) => {
    ...
    if (momentTime.format("YYYY-MM-DD") === currentDate) {
      time = t("common.dateTime.label.today");
    } else if (momentTime.add(1, "day").format("YYYY-MM-DD") === currentDate) {
      time = t("common.dateTime.label.yesterday");
    } else {
  ...
  return (
    ...
    >
      ...
      {routeTravel?.moments?.workMomentID ? (
        <div className="flex-1 overflow-y-auto pt-10 pb-3">
          <MomentsItemRender
        ...
        <Virtuoso
          ...
          components={{
            Header: () => (
              <>
                <div className="relative h-70 bg-[url(@/assets/images/moments/background.png)]">
                  <div
                  ...
                  >
                    ...
                    />
                    <div className="ml-2 max-w-50 truncate font-medium text-white">
                      {showUser?.nickname}
                    </div>
                  </div>
                </div>
                {Boolean(workMomentsUnreadCount) && (
                  <div className="my-3 flex items-center justify-center">
                    ...
                    >
                      {t("contact.text.multipleNewMessages", {
