...
import { usePublishMoments } from "@/api/hooks/moments";
import {
  FileWithPath,
  useFileMessage,
...
> = (
  ...
  const momentsPermissionMap = {
    [MomentPermission.Public]: t("contact.text.public"),
    [MomentPermission.Private]: t("contact.text.privacy"),
    [MomentPermission.AssignCanSee]: t("contact.text.partlyVisible"),
    [MomentPermission.AssignCanNotSee]: t("contact.text.hiddenSome"),
  };

  const { mutateAsync: publishMoments, isPending } = usePublishMoments(activeUserID);
  ...
  const confirmPublish = async () => {
    ...
    try {
      ...
      feedbackToast({ error, msg: t("contact.toast.publishFailed") });
    }
    closeOverlay();
  };
  ...
  const customUpload = async (option: UploadRequestOption) => {
    ...
    try {
      ...
      feedbackToast({ error, msg: t("system.toast.uploadFailed") });
    }
  };
        ...
        <div className="flex flex-1 flex-col">
          <div className="mt-2 bg-white p-2">
            <TextArea
              showCount
              bordered={false}
              placeholder={t("contact.text.publishMomentsToast")}
              className="mb-2"
              maxLength={150}
              value={text}
          ...
          >
            <div>
              <EyeFilled />
              <span className="ml-2">{t("contact.text.whoCanWatch")}</span>
            </div>
            <div className="flex max-w-[60%]">
              <div className="flex-1 truncate text-(--sub-text)">{getAssignStr()}</div>
              <RightOutlined className="text-(--sub-text)" />
            </div>
          </div>
          <div
          ...
          >
            <div>
              <BellFilled />
              <span className="ml-2">{t("contact.text.remindWhoToWatch")}</span>
            </div>
            <div className="flex max-w-[60%]">
              <div className="flex-1 truncate text-(--sub-text)">{getRemainStr}</div>
              <RightOutlined className="text-(--sub-text)" />
            </div>
          </div>
