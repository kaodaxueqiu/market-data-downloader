...
import { useClearUnreadMoments } from "@/api/hooks/moments";
import { MomentsClearType } from "@/api/services/moments";
...
import { broadcastClearMomentsUnreadCount } from "@/utils/window/broadcast";
...
> = (
  ...
  const { isPending: clearLoading, mutateAsync: clearUnreadMoments } =
  ...
  const messageListOpenChange = (visible: boolean) => {
    ...
    if (window.electronAPI?.enableCLib && workMomentsUnreadCount) {
      broadcastClearMomentsUnreadCount();
    }
  ...
  return (
    <div className="app-drag absolute top-0 z-10 flex w-full bg-linear-to-r from-[#0e2941] to-[#021b30] px-3 py-2">
