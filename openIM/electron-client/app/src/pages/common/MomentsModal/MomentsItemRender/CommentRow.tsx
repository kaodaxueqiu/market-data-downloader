...
import { useDeleteComment } from "@/api/hooks/moments";
...
const CommentRow = ({
  ...
  const { isPending, mutate } = useDeleteComment(activeUserID);
  ...
  const commentSuffix = comment.replyUserID
    ? ` ${t("common.text.reply")} ${comment.replyNickname}`
    : "";
  ...
  return (
    ...
    >
      ...
      >
        <span
          data-ignore
          className="mr-1.5 text-(--moment-text)"
