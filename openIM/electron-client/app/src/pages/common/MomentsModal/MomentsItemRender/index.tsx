...
import { useDeleteMoments, useLikeMoments } from "@/api/hooks/moments";
...
const MomentsItemRender = ({
  ...
  const { isPending: deleteLoading, mutateAsync: deleteMoments } =
    useDeleteMoments(activeUserID);
  const { isPending: likeLoading, mutateAsync: updateMomentsLike } =
  ...
  const likeSuffix =
    likeCount > 6
      ? t("contact.text.somePersonLike", { num: likeCount, othersNum: likeCount - 6 })
      : "";
  ...
  return (
    <div className="ignore-drag mx-5 mt-5 flex">
      ...
      <div className="ml-3 flex-1">
        <div
          className="w-fit max-w-50 truncate text-(--moment-text)"
        ...
        {Boolean(atUserStr) && (
          <div className="mb-2 text-xs text-(--sub-text)">{`${t(
            "contact.text.mentioned",
          )}：${atUserStr}`}</div>
        )}
        <div className="mb-2 flex w-full items-center justify-between">
          <div className="flex items-center">
            ...
            {isSelfMoments && (
              <div
                className="ml-3 cursor-pointer text-xs text-(--moment-text)"
        ...
        {Boolean(likeCount) && (
          <div className="mb-2 text-xs text-(--moment-text)">
            <img
              src={liked}
              width={12}
    ...
    return (
      <div className="flex rounded-sm bg-[#313e52]">
        <div className={actionStyle} onClick={updateLikeState}>
          ...
          <span className="ml-1">{t("contact.text.toLike")}</span>
        </div>
        <div className="my-1 border-r border-(--base-black)" />
