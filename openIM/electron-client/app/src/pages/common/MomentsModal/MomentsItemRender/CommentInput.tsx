...
import { useCreateComment } from "@/api/hooks/moments";
...
> = ({ workMomentID, updateRouteTravel, activeUserID }, ref) => {
  ...
  const confirmComment = () => {
    if (!commentState.text.trim()) {
      return message.warning(t("contact.toast.inputContent"));
    }
    mutateAsync({
      workMomentID,
  ...
  return (
    ...
    >
        ...
        <Input.TextArea
          ...
          placeholder={
            commentState.replyItem
              ? `${t("common.text.reply")}${commentState.replyItem.nickname}`
      ...
      <Button
        size="small"
        className="absolute right-4 bottom-4 mt-2 text-right"
        type="primary"
