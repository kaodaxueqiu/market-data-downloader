...
const CheckItem: FC<ICheckItemProps> = (props) => {
  ...
  return (
    <div
      className={clsx(
        "mx-2 flex items-center justify-between rounded-md px-3.5 py-2.5 hover:bg-(--primary-active)",
    ...
    >
      <div className="flex items-center">
        ...
        <OIMAvatar
          ...
          isdepartment={isDep}
        />
        <div className="ml-3 max-w-30 truncate">{showName}</div>
      </div>
      {showCheck ? (
        <RightOutlined className="cursor-pointer text-(--sub-text)" rev={undefined} />
      ) : (
        <CloseOutlined
