...
export interface YetiAnimationHandle {
  onInputFocus: (inputEl: HTMLInputElement) => void;
  onInputBlur: () => void;
...
const YetiAnimation = forwardRef<YetiAnimationHandle>((_, ref) => {
  ...
  const animateFace = useCallback((inputEl: HTMLInputElement) => {
    ...
    const screenCenter =
      svgCoords.x + svgContainerRef.current.offsetWidth / 2;
    const caretPos = caretCoords.x + emailCoords.x;
    const dFromC = screenCenter - caretPos;

    const inputY = emailCoords.y + 25;
    const inputX = emailCoords.x + caretCoords.x;
    ...
    const eyeLAngle = getAngle(eyeLCoords.x, eyeLCoords.y, inputX, inputY);
    const eyeRAngle = getAngle(eyeRCoords.x, eyeRCoords.y, inputX, inputY);
    const noseAngle = getAngle(noseCoords.x, noseCoords.y, inputX, inputY);
    ...
    const dur = 1;
    const ease = "expo.out";
    gsap.to(eyeLRef.current, { duration: dur, x: -eyeLX, y: -eyeLY, ease });
    gsap.to(eyeRRef.current, { duration: dur, x: -eyeRX, y: -eyeRY, ease });
  ...
  const resetFace = useCallback(() => {
    ...
    gsap.to(
      ...
      { duration: dur, x: 0, y: 0, scaleY: 1, ease },
    );
  }, []);
  ...
  useImperativeHandle(
    ref,
    () => ({
      onInputFocus: (inputEl: HTMLInputElement) => animateFace(inputEl),
      onInputBlur: () => resetFace(),
      onInputChange: (inputEl: HTMLInputElement) => animateFace(inputEl),
      onPasswordFocus: () => coverEyes(),
      onPasswordBlur: () => uncoverEyes(),
    }),
    [animateFace, resetFace, coverEyes, uncoverEyes],
  );
    ...
    >
      ...
      >
        ...
        >
          ...
          {/* right ear */}
          <g className="earR">
            <g ref={outerEarRRef} fill="#ddf1fa" stroke="#3a5e77" strokeWidth="2.5">
          ...
          <path
            ...
            strokeLinecap="round"
            strokeLinejoin="round"
            d="M81.457,27.929c1.755-4.084,5.51-8.262,11.253-11.77c0.979,2.565,1.883,5.14,2.712,7.723
            c3.162-4.265,8.626-8.27,16.272-11.235c-0.737,3.293-1.588,6.573-2.554,9.837c4.857-2.116,11.049-3.64,18.428-4.156
          ...
          <g ref={eyebrowRef} className="eyebrow">
            ...
            <path
              ...
              strokeLinecap="round"
              strokeLinejoin="round"
              d="M63.56,55.102c6.243,5.624,13.38,10.614,21.296,14.738c2.071-2.785,4.01-5.626,5.816-8.515
              c4.537,3.785,9.583,7.263,15.097,10.329c1.197-3.043,2.287-6.104,3.267-9.179c4.087,2.004,8.427,3.761,12.996,5.226
              c0.545-3.348,0.986-6.696,1.322-10.037c4.913-0.481,9.857-1.34,14.787-2.599"
            />
          </g>
