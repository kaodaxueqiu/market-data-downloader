import { base64toFile, getFileType } from "@openim/shared";
import { MessageItem } from "@openim/wasm-client-sdk";
import { v4 as uuidV4 } from "uuid";

import { IMSDK } from "@/layout/MainContentWrap";
import { ExMessageItem } from "@/store";
import { canSendImageTypeList } from "@/utils/file";
...
export function useFileMessage() {
  const getImageMessage = async (file: FileWithPath) => {
    ...
    if (window.electronAPI?.enableCLib) {
      return (await IMSDK.createImageMessageFromFullPath(sourcePath))
        .data as MessageItem;
    }
    const options = {
