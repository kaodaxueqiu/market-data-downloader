...
const CustomEmojiItemEl = ({
  ...
  return (
    <div
      className={clsx("h-11.5 w-11.5 rounded-md", {
      ...
      <Popover
        content={
          <div className="p-1">
            ...
            >
              <div className="text-xs text-red-400">{t("chat.action.delete")}</div>
            </div>
          </div>
        }
...
const CustomTabPane = ({ closeAllPop }: { closeAllPop?: () => void }) => {
  ...
  const fileHandle = async (options: UploadRequestOption) => {
    ...
    if (customEmojis.some((item) => item.url === url)) {
  ...
  return (
    <div className="my-5 grid flex-1 grid-cols-6 gap-3 overflow-auto px-5">
      ...
      >
        <img
          className="min-w-11.5 cursor-pointer"
          width={46}
          src={favorite_add}
          alt=""
        />
...
const EmojiPopContent = ({
  ...
  if (!pickerRef.current) {
    pickerRef.current = createEmojiPicker();
  }
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (emojiPicker.parentElement === ref.current) {
        ref.current?.removeChild(emojiPicker);
      }
    };
  }, []);

  return (
    <div className="flex h-64 w-95 flex-col">
      <div
        ref={ref}
