...
const debugLog = (...args: any[]) => console.debug("[MediaPreview]", ...args);
...
> = ({ conversationID }, ref) => {
  ...
  const downloadOrShowFinder = (current: number) => {
    ...
    debugLog("downloadOrShowFinder", {
      clientMsgID,
      sourceUrl,
      hasTask: Boolean(currentTask),
      downloadState: currentTask?.downloadState,
    });
    downloadFile(
      sourceUrl,
      buildMessageDownloadData({
  ...
  const showAlbum = async (clientMsgID: string) => {
    ...
    if (current < 0) {
      ...
      const refreshedList = [...useMessageStore.getState().previewImgList].reverse();
      current = refreshedList.findIndex((img) => img.clientMsgID === clientMsgID);
      findItem = refreshedList[current];
    }
    if (current < 0) return;
    if (!findItem) return;
    const sourceUrl = findItem.videoUrl ?? findItem.url;
    debugLog("showAlbum trigger download", { clientMsgID, sourceUrl });
...
const PreviewItemRender = ({
  ...
  const tryPlayVideo = () => {
    ...
    if (!currentTask && source.videoUrl) {
      debugLog("tryPlayVideo start download", {
        clientMsgID: source.clientMsgID,
