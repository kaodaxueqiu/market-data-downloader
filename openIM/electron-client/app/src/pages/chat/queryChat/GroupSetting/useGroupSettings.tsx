...
export function useGroupSettings({ closeOverlay }: { closeOverlay: () => void }) {
  ...
  const updateGroupInfo = useCallback(
    async (value: Partial<GroupItem>) => {
      ...
      try {
        ...
        feedbackToast({ error, msg: t("chat.toast.updateGroupInfoFailed") });
      }
    },
  ...
  const updateGroupVerification = useCallback(
    async (verification: GroupVerificationType) => {
      ...
      try {
        ...
        feedbackToast({ error, msg: t("chat.toast.updateGroupVerificationFailed") });
      }
    },
    [currentGroupInfo?.groupID],
  );
      ...
      modalRef.current = modal.confirm({
        ...
        content: (
          <div className="flex items-baseline">
            <div>{t("chat.toast.confirmAllMuted")}</div>
            <span className="text-xs text-(--sub-text)">
              {t("chat.group.onlyManageCanSend")}
            </span>
          </div>
        ),
        onOk: execFunc,
  ...
  const tryDismissGroup = () => {
    ...
    modalRef.current = modal.confirm({
      title: t("chat.group.disbandGroup"),
      content: (
        <div className="flex items-baseline">
          <div>{t("chat.toast.confirmDisbandGroup")}</div>
          <span className="text-xs text-(--sub-text)">
            {t("chat.group.disbandGroupToast")}
          </span>
        </div>
      ),
      onOk: async () => {
  ...
  const tryQuitGroup = () => {
    ...
    modalRef.current = modal.confirm({
      title: t("chat.group.exitGroup"),
      content: (
        <div className="flex items-baseline">
          <div>{t("chat.toast.confirmExitGroup")}</div>
          <span className="text-xs text-(--sub-text)">
            {t("chat.group.exitGroupToast")}
          </span>
        </div>
      ),
      onOk: async () => {
