  ...
  return (
    <div className="p-4">
      <div className="mb-3 font-medium">
        <span>{t("chat.group.groupMember")}</span>
      ...
      <div className="flex flex-wrap items-center">
        {fetchState.groupMemberList.slice(0, sliceCount).map((member) => (
          ...
          >
            ...
            <div className="mt-2 min-h-4 max-w-full truncate text-xs">
              {member.nickname}
            </div>
          </div>
        ))}
        ...
        >
          ...
          <div className="mt-2 max-w-full truncate text-xs text-(--sub-text)">
            {t("chat.action.add")}
          </div>
        </div>
        {!isNomal && (
      ...
      <div
        className="text-primary flex cursor-pointer items-center justify-center pt-2 text-xs"
        onClick={updateTravel}
      >
        {t("common.text.viewMore")}
      </div>
    </div>
  );
};
