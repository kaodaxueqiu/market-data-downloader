...
import VoiceToTextBubble from "./VoiceToTextBubble";
...
const MessageItem: FC<IMessageItemProps> = ({
  ...
  const { isVoiceToTextLoading, hasVoiceText } = useVoiceToText({
    message,
  ...
  const { mutate: markGroupMessageAsRead, isPending: markGroupMessageAsReadLoading } =
    useMutation<
      unknown,
      unknown,
      Parameters<typeof IMSDK.sendGroupMessageReadReceipt>[0]
    >({
      mutationFn: (params) => IMSDK.sendGroupMessageReadReceipt(params),
    });
  ...
  return (
    <>
      {message.gapTime && (
        <div className="py-2 text-center text-xs text-(--sub-text)">
      ...
      <div
        ...
        className={clsx(
          "relative flex px-5 py-3 select-text",
      ...
      >
        ...
        >
          ...
          >
            <OIMAvatar
              size={36}
              src={agentInfo?.faceURL || message.senderFaceUrl}
              text={agentInfo?.nickname || message.senderNickname}
          ...
          <div className={styles["message-wrap"]} ref={messageWrapRef}>
            <div className={styles["message-profile"]}>
              {!isSender && (
                <span
                  title={agentInfo?.nickname || message.senderNickname}
                  className={clsx("max-w-[30%] truncate")}
                >
                  {agentInfo?.nickname || message.senderNickname}
                </span>
              )}
            ...
            >
              ...
              <div
                className={clsx(
                  "invisible mt-3.5 text-xs text-(--sub-text)",
                  isHovering && "visible!",
                )}
              >
            ...
            {message.contentType === MessageType.VoiceMessage &&
              (hasVoiceText || isVoiceToTextLoading) && (
                <VoiceToTextBubble conversationID={conversationID!} message={message} />
              )}
        ...
        {(Number(message.errCode) as IMSDKErrCode) === IMSDKErrCode.Blacked && (
          <div className="absolute bottom-1 left-1/2 flex -translate-x-1/2 items-center text-xs">
            <div className="text-xs text-(--sub-text)">
              {t("chat.toast.beingBlacklist")}
            </div>
            {(Number(message.errCode) as IMSDKErrCode) === IMSDKErrCode.NotFriend && (
              <>
                <div className="text-(--sub-text)">
                  {t("chat.toast.usingFriendVerification")}
                </div>
                <div className="text-primary ml-2 cursor-pointer">
                  {t("chat.contact.verifyAdd")}
                </div>
              </>
            )}
          </div>
...
const MessageAvatarMenuContent = memo(
  ({
    ...
    return (
      <div className="p-1">
        <div
          className="max-w-30 cursor-pointer truncate rounded px-3 py-2 text-xs hover:bg-(--primary-active)"
          onClick={triggerAt}
        >
          {`@${senderNickname}`}
        </div>
