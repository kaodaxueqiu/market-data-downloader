import { secondsToMS } from "@openim/shared";
...
const MediaMessageRender: FC<IMessageItemProps> = ({
  ...
  const localPath = resolveMessageLocalPath(message);
  const isFileExists = Boolean(localPath);

  return (
    <Spin spinning={isSending} tip={`${uploadProgress}%`}>
      <div className="relative max-w-50" style={minStyle} ref={dragRef}>
        <CacheImage
        ...
        />
        {isVideoMessage && (
          <div className="absolute right-4 bottom-3 text-white">
