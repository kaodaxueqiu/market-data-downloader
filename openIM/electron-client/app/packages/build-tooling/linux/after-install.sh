...
detect_and_create() {
  for dir in /opt/*; do
    ...
    # Only handle Electron-style app dirs containing app.asar or asar unpacked.
    if [ ! -d "$resources_dir" ]; then
      continue
    fi
    if [ ! -f "$resources_dir/app.asar" ] && [ ! -d "$resources_dir/app.asar.unpacked" ]; then
      # also accept any *.asar file inside resources as signal
    ...
    # Prefer binary with same basename as directory.
    if [ -x "$dir/$product_name" ]; then
      exec_name="$product_name"
    else
      # Fallback: first executable at top-level (skip chrome-sandbox and obvious libs).
    ...
    create_desktop_entry "$product_name" "$exec_name" "$product_name"
  done
}
