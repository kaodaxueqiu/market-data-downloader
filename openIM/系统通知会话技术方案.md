...
> 事实来源：前端 `openIM/electron-client/app/src` 实测 + 后端 `192.168.30.11` OpenIM 企业版部署实测。
...
| 置顶 `pinConversation({conversationID,isPinned})` | `pages/chat/ConversationSider/ConversationMenuContent.tsx:69` / `hooks/useConversationSettings.ts:23` 已用 | ✅ |
| 通知消息渲染 `SystemNotification` | `pages/chat/queryChat/SystemNotification.tsx`（含内部 `NotificationRender`/`MediaRow`） | ✅ |
...
### 1. 常量（新增）
`openIM/electron-client/app/src/constants/im.ts`
```ts
export const SYSTEM_NOTIFICATION_USER_ID = "system_notification"; // 与后端账号 ID 一致
```

### 2. 登录同步完成后：确保会话存在并置顶（主改动）
`openIM/electron-client/app/src/layout/useGlobalEvents.tsx` 的 `syncFinishHandler`（第 463 行起）。
...
### 3. 绝对第一行（排序特判）
`openIM/electron-client/app/src/utils/imCommon.ts:629` `conversationSort`。
现状：`isPinned` 优先，但**多个置顶项之间按 `max(draftTextTime, latestMsgSendTime)` 排**——无法保证系统通知恒在最顶。
...
`openIM/electron-client/app/src/pages/chat/ConversationSider/ConversationItem.tsx`
- 标题 `{conversation.showName}` 与 `OIMAvatar` 的 `src`/`text`，对 `conversation.userID === SYSTEM_NOTIFICATION_USER_ID` 强制显示「系统通知」名称与专属图标。
- 已有先例：当前 agent 会话用 `agentInfo?.nickname || conversation.showName` override（见 `OIMAvatar` 的 `text` 与 `src`），照此模式即可。
- 注：该组件已有 `isNotification = conversation.conversationType === SessionType.Notification` 判断与 `isnotification` avatar 属性，可直接复用。

### 5. 禁止删除/取消置顶（UI 定制）
`openIM/electron-client/app/src/pages/chat/ConversationSider/ConversationMenuContent.tsx`
- 对系统通知会话，隐藏「置顶/取消置顶」MenuItem（L153-160）与底部红色「隐藏」MenuItem（L210-214，标题硬编码中文「隐藏」，调用 `hideConversation`），保证其真正固定、不可被用户移除。
