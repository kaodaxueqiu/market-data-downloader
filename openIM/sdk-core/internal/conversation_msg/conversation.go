...
func (c *Conversation) setConversation(ctx context.Context, apiReq *pbConversation.SetConversationsReq, localConversation *model_struct.LocalConversation) error {
	...
	err := api.SetConversations.Execute(ctx, apiReq)
	if err != nil {
		return err
	}
	err = c.conversationHotColdManager.Promote(ctx, localConversation.ConversationID)
	if err != nil {
		log.ZWarn(ctx, "promote conversation to hot storage failed", err, "conversationID", localConversation.ConversationID)
	}
	//if apiReq.Conversation != nil {
	//	if apiReq.Conversation.ConversationID != "" {
	//		_ = c.OnConversationChanged(ctx, []string{apiReq.Conversation.ConversationID})
	//	}
	//}
	return nil
}
...
func (c *Conversation) getAdvancedHistoryMessageList(ctx context.Context, req sdk.GetAdvancedHistoryMessageListParams, isReverse bool) (*sdk.GetAdvancedHistoryMessageListCallback, error) {
	...
	log.ZDebug(ctx, "pull message", "pull cost time", time.Since(t).String())
	t = time.Now()
	...
	go c.groupMessageHasReadInfo(ctx, conversationID, &messageList, isReverse, req.ViewType)
...
func (c *Conversation) fetchMessagesWithGapCheck(ctx context.Context, conversationID string,
	...
	shouldFetchMoreMessagesNum := func(messages []*model_struct.LocalChatLog) int {
		...
		for _, msg := range messages {
			...
			if msg.Status >= constant.MsgStatusHasDeleted && msg.Status != constant.MsgStatusCanceled {
...
func (c *Conversation) LocalChatLog2MsgStruct(list []*model_struct.LocalChatLog) []*sdk_struct.MsgStruct {
	...
	for _, v := range list {
		temp := converter.LocalChatLogToMsgStruct(v)
...
func (c *Conversation) searchLocalMessages(ctx context.Context, searchParam *sdk.SearchLocalMessagesParams) (*sdk.SearchLocalMessagesCallback, error) {
	...
	// Search in a specific conversation if ConversationIDs is provided
