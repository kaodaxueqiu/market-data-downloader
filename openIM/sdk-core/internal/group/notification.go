...
func (g *Group) doGroupNotificationPrune(ctx context.Context, value *sdkws.GroupNotificationPrune) error {
	for _, groupID := range value.GroupIDs {
		g.memberSyncManager.ForceEnsureAsync(ctx, groupID)
	}
	return nil
}
	...
	case constant.GroupApplicationAcceptedNotification: // 1505
		...
		if g.filter.ShouldExecute(detail.Uuid) {
			g.listener().OnGroupApplicationAccepted(utils.StructToJsonString(
				converter.ServerGroupRequestToLocalForNotification(detail.GetGroup(), detail.GetRequest())))
		}
