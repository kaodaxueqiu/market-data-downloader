...
func (c *Third) SetTemporaryLevel(ctx context.Context, logLevel int, seconds int) error {
	return log.SetTemporaryLevel(logLevel, time.Duration(seconds)*time.Second)
}
