...
func (it *SortConversationIterator) NextTop(size int) []*ConversationMetaData {
	if it.list == nil || it.list.conversations == nil {
		return nil
	}
...
func (p *ConversationBatchProcessor) Run(ctx context.Context, handler BatchHandler) {
	...
	for {
		// Exit if context is canceled (support cancellation)
		if err := ctx.Err(); err != nil {
			break
		}
		batch := p.iter.NextTop(p.batchSize)

		// ✅ If no data left, finalize and exit
		if len(batch) == 0 {
			if len(result) > 0 {
				p.emitResult(ctx, handler, result)
			} else if p.isFirst {
				// If this is the first run and nothing fetched,
				// still emit an empty result once
				p.emitResult(ctx, handler, result)
			}
			break
		}
		...
		for _, conv := range batch {
			...
			if nid := GetNotificationConversationIDByConversationID(conv.ConversationID); nid != "" {
				if v, ok := p.needSyncSeqMap[nid]; ok {
					result[nid] = v
				}
			}
