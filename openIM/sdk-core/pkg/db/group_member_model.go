...
func (d *DataBase) GetGroupMemberListSplit(ctx context.Context, groupID string, filter int32, offset, count int) ([]*model_struct.LocalGroupMember, error) {
	...
	case constant.GroupFilterOwnerAndAdmin:
		err = d.conn.WithContext(ctx).Where("group_id = ? And (role_level = ? or role_level = ?)", groupID,
			constant.GroupOwner, constant.GroupAdmin).Order("role_level DESC,join_time ASC").Offset(offset).
			Limit(count).Find(&groupMemberList).Error
	case constant.GroupFilterNotSelf:
		err = d.conn.WithContext(ctx).Where("group_id = ? and user_id != ?", groupID, d.loginUserID).
			Offset(offset).Limit(count).Find(&groupMemberList).Error
	default:
		return nil, errs.New("filter args failed", "filter", filter).Wrap()
	}
