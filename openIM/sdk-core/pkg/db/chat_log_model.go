...
func (d *DataBase) GetMessageByUserID(ctx context.Context, conversationID string, userID string) ([]*model_struct.LocalChatLog, error) {
	err := d.initChatLog(ctx, conversationID)
	if err != nil {
		log.ZWarn(ctx, "initChatLog err", err)
		return nil, err
	...
	return result, errs.WrapMsg(d.conn.WithContext(ctx).Table(utils.GetTableName(conversationID)).Where("send_id = ?",
		userID).Find(&result).Error, "GetMessage failed")
}
...
func (d *DataBase) GetMessageBySeq(ctx context.Context, conversationID string, seq int64) (*model_struct.LocalChatLog, error) {
	...
	defer d.mRWMutex.RUnlock()
	var c model_struct.LocalChatLog
	dbErr := d.conn.WithContext(ctx).Table(utils.GetTableName(conversationID)).Where("seq = ?", seq).Take(&c).Error
	if dbErr != nil {
		if errors.Is(dbErr, gorm.ErrRecordNotFound) {
			return nil, errs.ErrRecordNotFound.Wrap()
		}
		return nil, errs.WrapMsg(dbErr, "GetMessage failed")
	}
	return &c, nil
}
...
func (d *DataBase) SearchMessageByContentType(ctx context.Context, contentType []int, senderUserIDList []string, conversationID string, startTime, endTime int64, offset, count int) (result []*model_struct.LocalChatLog, err error) {
	...
	if len(senderUserIDList) > 0 {
		db = db.Where("send_id IN ?", senderUserIDList)
	}

	err = errs.WrapMsg(
		db.Order("send_time DESC").
			Offset(offset).
			Limit(count).
			Find(&result).Error,
...
func (d *DataBase) SearchMessageByKeyword(ctx context.Context, contentType []int, senderUserIDList []string,
	keywordList []string, keywordListMatchType int, conversationID string, startTime, endTime int64, offset, count int) (result []*model_struct.LocalChatLog, err error) {
	d.mRWMutex.RLock()
	defer d.mRWMutex.RUnlock()
...
func (d *DataBase) DeleteMessagesByClientMsgIDs(ctx context.Context, conversationID string, msgIDs []string) (rowsAffected int64, err error) {
	d.mRWMutex.Lock()
	defer d.mRWMutex.Unlock()

	t := d.conn.WithContext(ctx).Table(utils.GetConversationTableName(conversationID)).
		Where("client_msg_id in ? ", msgIDs).Update("status", constant.MsgStatusHasDeleted)

	if t.RowsAffected == 0 {
		return 0, errs.WrapMsg(errors.New("RowsAffected == 0"), "no messages updated")
	}

	return t.RowsAffected, errs.WrapMsg(t.Error, "Failed to update message status")
}
