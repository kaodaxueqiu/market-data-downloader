...
func CreateConversationGroup(callback open_im_sdk_callback.Base, operationID string, req string) {
	call(callback, operationID, IMUserContext.Conversation().CreateConversationGroup, req)
}

func UpdateConversationGroup(callback open_im_sdk_callback.Base, operationID string, req string) {
	call(callback, operationID, IMUserContext.Conversation().UpdateConversationGroup, req)
}

func DeleteConversationGroup(callback open_im_sdk_callback.Base, operationID string, conversationGroupID string) {
	call(callback, operationID, IMUserContext.Conversation().DeleteConversationGroup, conversationGroupID)
}

func GetConversationGroups(callback open_im_sdk_callback.Base, operationID string, conversationGroupType int64) {
	call(callback, operationID, IMUserContext.Conversation().GetConversationGroups, conversationGroupType)
}

func SetConversationGroupOrder(callback open_im_sdk_callback.Base, operationID string, orders string) {
	call(callback, operationID, IMUserContext.Conversation().SetConversationGroupOrder, orders)
}

func AddConversationsToGroups(callback open_im_sdk_callback.Base, operationID string, conversationIDs string, conversationGroupIDs string) {
	call(callback, operationID, IMUserContext.Conversation().AddConversationsToGroups, conversationIDs, conversationGroupIDs)
}

func RemoveConversationsFromGroups(callback open_im_sdk_callback.Base, operationID string, conversationIDs string, conversationGroupIDs string) {
	call(callback, operationID, IMUserContext.Conversation().RemoveConversationsFromGroups, conversationIDs, conversationGroupIDs)
}

func GetConversationGroupByConversationID(callback open_im_sdk_callback.Base, operationID string, conversationID string) {
	call(callback, operationID, IMUserContext.Conversation().GetConversationGroupByConversationID, conversationID)
}

func GetConversationGroupInfoWithConversations(callback open_im_sdk_callback.Base, operationID string, req string) {
	call(callback, operationID, IMUserContext.Conversation().GetConversationGroupInfoWithConversations, req)
}
...
func CreateMarkdownMessage(operationID string, content string) string {
	return syncCall(operationID, IMUserContext.Conversation().CreateMarkdownMessage, content)
}
...
// deprecated
func CreateTargetedGroupMessage(operationID string, message, dstUserIDs string) string {
	return syncCall(operationID, IMUserContext.Conversation().CreateTargetedGroupMessage, message, dstUserIDs)
}
...
func SendGroupMessageReadReceipt(callback open_im_sdk_callback.Base, operationID string, conversationID string,
	clientMsgIDs string) {
	call(callback, operationID, IMUserContext.Conversation().SendGroupMessageReadReceipt, conversationID, clientMsgIDs)
}
func GetGroupMessageReaderList(callback open_im_sdk_callback.Base, operationID string, conversationID string, clientMsgID string, filter, offset, count int32) {
	call(callback, operationID, IMUserContext.Conversation().GetGroupMessageReaderList, conversationID, clientMsgID, filter, offset, count)
}
...
func DeleteMessages(callback open_im_sdk_callback.Base, operationID string, req string) {
	call(callback, operationID, IMUserContext.Conversation().DeleteMessages, req)
}

func DeleteUserAllMessagesInConv(callback open_im_sdk_callback.Base, operationID string, conversationID string, userID string) {
	call(callback, operationID, IMUserContext.Conversation().DeleteUserAllMessagesInConv, conversationID, userID)
}
...
func SetMessageLocalContent(callback open_im_sdk_callback.Base, operationID string, conversationID, message string) {
	call(callback, operationID, IMUserContext.Conversation().SetMessageLocalContent, conversationID, message)
}
...
func FetchSurroundingMessages(callback open_im_sdk_callback.Base, operationID string, req string) {
	call(callback, operationID, IMUserContext.Conversation().FetchSurroundingMessages, req)
}

func ModifyMessage(callback open_im_sdk_callback.Base, operationID string, req string) {
	call(callback, operationID, IMUserContext.Conversation().ModifyMessage, req)
}

func SetConversationPinnedMsg(callback open_im_sdk_callback.Base, operationID string, conversationID string, clientMsgID string, pinned bool) {
	call(callback, operationID, IMUserContext.Conversation().SetConversationPinnedMsg, conversationID, clientMsgID, pinned)
}

func GetConversationPinnedMsg(callback open_im_sdk_callback.Base, operationID string, conversationID string) {
	call(callback, operationID, IMUserContext.Conversation().GetConversationPinnedMsg, conversationID)
}

func ResetConversationUnread(callback open_im_sdk_callback.Base, operationID string, req string) {
	call(callback, operationID, IMUserContext.Conversation().ResetConversationUnread, req)
}
