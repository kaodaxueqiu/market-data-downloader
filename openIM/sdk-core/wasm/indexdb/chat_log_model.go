...
func (i *LocalChatLogs) GetMessageByUserID(ctx context.Context, conversationID string, userID string) ([]*model_struct.LocalChatLog, error) {
	msg, err := exec.Exec(conversationID, userID)
	if err != nil {
		return nil, err
	} else {
		if v, ok := msg.(string); ok {
...
func (i *LocalChatLogs) DeleteMessagesByClientMsgIDs(ctx context.Context, conversationID string, msgIDs []string) (rowsAffected int64, err error) {
	rows, err := exec.Exec(conversationID, utils.StructToJsonString(msgIDs))
	if err != nil {
		return 0, err
	} else {
		if v, ok := rows.(float64); ok {
