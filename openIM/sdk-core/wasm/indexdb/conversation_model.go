...
func (i *LocalConversations) GetConversationUnreadCountMap(ctx context.Context, conversationIDs []string) (map[string]int32, error) {
	result := make(map[string]int32)
	cList, err := exec.Exec(utils.StructToJsonString(conversationIDs))
	if err != nil {
		return nil, err
	}
	if v, ok := cList.(string); ok {
...
func (i *LocalConversations) GetFilterConversationIDsDB(ctx context.Context, groupID string) (result []string, err error) {
...
func (i *LocalConversations) GetFilterConversationUnreadCountDB(ctx context.Context, groupID string) (totalUnreadCount int32, err error) {
...
func (i *LocalConversations) GetFilterConversationPageDB(ctx context.Context, groupID string, offset, count int) (result []*model_struct.LocalConversation, total int64, err error) {
	page, err := exec.Exec(groupID, offset, count)
	if err != nil {
		return nil, 0, err
	}
	if v, ok := page.(string); ok {
		var payload struct {
			Result []model_struct.LocalConversation `json:"result"`
			Total  int64                            `json:"total"`
		}
		err := utils.JsonStringToStruct(v, &payload)
		if err != nil {
			return nil, 0, err
		}
		for _, conversation := range payload.Result {
			conversationCopy := conversation
			result = append(result, &conversationCopy)
		}
		return result, payload.Total, nil
	}
	return nil, 0, exec.ErrType
}
...
func (i *LocalConversations) UpdateOrCreateConversations(ctx context.Context, conversationList []*model_struct.LocalConversation) error {
	...
	//			if utils.IsContain(v.ConversationIDs, conversationIDs) {
