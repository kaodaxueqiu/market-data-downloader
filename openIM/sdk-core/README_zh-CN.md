...
- Update the following fields with your server information:
  ...
  ```dotenv
  OPENIM_APIADDR=http://your-server-api-address
  OPENIM_WSADDR=ws://your-server-websocket-address
  OPENIM_USER_ID=your-test-user-id
  OPENIM_PLATFORM_ID=5
  OPENIM_SECRET=your-admin-secret
  OPENIM_ADMIN_USER_ID=your-admin-user-id
  ```
  - Use `OPENIM_SECRET` and `OPENIM_ADMIN_USER_ID` only for local testing. Do not store or commit them for production use.
