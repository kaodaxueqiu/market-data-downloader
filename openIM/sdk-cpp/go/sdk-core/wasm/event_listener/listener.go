...
func NewConversationGroupCallback(callback *js.Value) *ConversationGroupCallback {
	return &ConversationGroupCallback{CallbackWriter: NewEventData(callback)}
}

func (c ConversationGroupCallback) OnConversationGroupAdded(conversationGroupList string) {
	c.CallbackWriter.SetEvent(utils.GetSelfFuncName()).SetData(conversationGroupList).SendMessage()
}
...
func (a AdvancedMsgCallback) OnDeleteUserAllMsgsInConv(message string) {
	a.CallbackWriter.SetEvent(utils.GetSelfFuncName()).SetData(message).SendMessage()
}
