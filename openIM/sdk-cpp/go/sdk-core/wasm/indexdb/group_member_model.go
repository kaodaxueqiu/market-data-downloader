...
func NewLocalGroupMember(loginUserID string) *LocalGroupMember {
	return &LocalGroupMember{loginUserID: loginUserID}
}
...
func (i *LocalGroupMember) GetGroupMemberListSplit(ctx context.Context, groupID string, filter int32, offset, count int) ([]*model_struct.LocalGroupMember, error) {
	member, err := exec.Exec(groupID, filter, offset, count, i.loginUserID)
	if err != nil {
		return nil, err
	} else {
		if v, ok := member.(string); ok {
