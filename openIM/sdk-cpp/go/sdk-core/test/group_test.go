...
func Test_GetSpecifiedGroupsInfo(t *testing.T) {
	info, err := open_im_sdk.IMUserContext.Group().GetSpecifiedGroupsInfo(ctx, []string{"2695472114"})
	if err != nil {
		t.Fatal(err)
	}
...
func Test_GetGroupMemberList(t *testing.T) {
	...
	members, err := open_im_sdk.IMUserContext.Group().GetGroupMemberList(ctx, "2695472114", 0, 0, 9999999)
	if err != nil {
		panic(err)
	}
...
func TestGetGroupMember(t *testing.T) {
	...
	lgm, err := g.GetGroupMemberList(ctx, "2695472114", constant.GroupFilterAll, 0, 100)
	if err != nil {
		t.Log(err)
		return
	}
