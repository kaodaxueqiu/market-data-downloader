...
func (r *Relation) GetSpecifiedFriendsInfo(ctx context.Context, friendUserIDList []string, filterBlack bool) ([]*model_struct.LocalFriend, error) {
	dataFetcher := datafetcher.NewDataFetcher(
		...
		func(ctx context.Context, userIDs []string) ([]*model_struct.LocalFriend, error) {
			...
			return datautil.Batch(converter.ServerFriendToLocal, serverFriend), nil
		},
	)
...
func (r *Relation) GetFriendApplicationListAsRecipient(ctx context.Context, req *sdk.GetFriendApplicationListAsRecipientReq) ([]*model_struct.LocalFriendRequest, error) {
	...
	return datautil.Batch(converter.ServerFriendRequestToLocal, friendRequests), nil
}
...
func (r *Relation) DeleteFriendRequests(ctx context.Context, req *sdk.DeleteFriendRequestsReq) error {
	var (
		fromUserReq = &relation.DeleteFriendRequestsFromUserReq{
			FromUserID: r.loginUserID,
			ToUserIDs:  make([]string, 0),
		}
		toUserReq = &relation.DeleteFriendRequestsToUserReq{
			ToUserID:    r.loginUserID,
			FromUserIDs: make([]string, 0),
		}
	)

	for _, r1 := range req.FriendRequests {
		if r1.FromUserID == r.loginUserID {
			fromUserReq.ToUserIDs = append(fromUserReq.ToUserIDs, r1.ToUserID)
		} else if r1.ToUserID == r.loginUserID {
			toUserReq.FromUserIDs = append(toUserReq.FromUserIDs, r1.FromUserID)
		} else {
			log.ZWarn(ctx, "can`t delete others friend request", nil)
		}
	}

	gr := errgroup.Group{}
	if len(fromUserReq.ToUserIDs) != 0 {
		gr.Go(func() error {
			if err := r.deleteFriendRequestsFromUser(ctx, fromUserReq); err != nil {
				return err
			}
			return nil
		})
	}
	if len(toUserReq.FromUserIDs) != 0 {
		gr.Go(func() error {
			if err := r.deleteFriendRequestsToUser(ctx, toUserReq); err != nil {
				return err
			}
			return nil
		})
