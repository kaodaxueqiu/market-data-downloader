...
type LongConnMgr struct {
	...
	groupMgrCh         chan common.Cmd2Value
	closedErr          error
	ctx                context.Context
	IsCompression      bool
	Syncer             *WsRespAsyn
...
func NewLongConnMgr(ctx context.Context, userOnline func(map[string][]int32), pushMsgAndMaxSeqCh, loginMgrCh, groupMgrCh chan common.Cmd2Value) *LongConnMgr {
	l := &LongConnMgr{
		userOnline:         userOnline,
		pushMsgAndMaxSeqCh: pushMsgAndMaxSeqCh,
		loginMgrCh:         loginMgrCh,
		groupMgrCh:         groupMgrCh,
		IsCompression:      true,
...
func (l *LongConnMgr) SetLoginUserID(loginUserID string) {
	l.userID = loginUserID
}
...
func (c *LongConnMgr) readPump(ctx context.Context, fgCtx context.Context) {
	...
	for {
		...
		if err != nil {
			dur := c.reconnectStrategy.GetSleepInterval()
			log.ZWarn(c.ctx, "reConn", err, "sleep", dur)
			timer := time.NewTimer(dur)
			select {
			case <-ctx.Done():
			case <-timer.C:
			case <-c.netChanged:
				log.ZDebug(c.ctx, "network update triggered reConn")
			}
			timer.Stop()
			continue
		}
...
func (c *LongConnMgr) heartbeat(ctx context.Context, fgCtx context.Context) {
	...
	for {
		...
		case <-fgCtx.Done():
			...
			return
		case <-ticker.C:
			log.ZInfo(ctx, "sendPingMessage", "goroutine ID:", getGoroutineID(), "addr", c.conn.LocalAddr())
...
func (c *LongConnMgr) handleMessage(message []byte) error {
	...
	ctx = context.WithValue(ctx, "logUserID", c.userID)
	...
	case constant.KickOnlineMsg:
		...
		if len(wsResp.Data) > 0 {
			...
			if err := proto.Unmarshal(wsResp.Data, &tips); err == nil {
				...
				log.ZError(ctx, "unmarshal ForceLogoutTips", err)
			}
		}
		ccontext.GetApiErrCodeCallback(ctx).OnError(ctx, err, attr)
		return err
	...
	case constant.WsGroupNotificationPrune:
		if err := c.doWsGroupNotificationPrune(ctx, wsResp); err != nil {
			log.ZError(ctx, "doWsGroupNotificationPrune failed", err, "wsResp", wsResp)
		}
	default:
...
func (c *LongConnMgr) reConn(ctx context.Context, num *int) (needRecon bool, err error) {
	...
	diagnose.Info("ws.reconnect.start",
		"opid", info.OperationID(),
		"background", fmt.Sprint(c.GetBackground()),
		"ws", info.WsAddr(),
	)
	req := map[string]any{
		"userID":                 info.UserID(),
	...
	//urlval := fmt.Sprintf("%s?sendID=%s&token=%s&platformID=%d&operationID=%s&isBackground=%t",
	...
	//if c.IsCompression {
	//	urlval += fmt.Sprintf("&compression=%s", "gzip")
	//}
	...
	if err != nil {
		...
		if resp != nil {
			...
			if err != nil {
				diagnose.Warn("ws.reconnect.fail", err,
					"opid", info.OperationID(),
				)
				return true, err
			}
			...
			ccontext.GetApiErrCodeCallback(ctx).OnError(ctx, err, nil)
			diagnose.Warn("ws.reconnect.fail", err,
				"opid", info.OperationID(),
			)
	...
	diagnose.Info("ws.reconnect.success",
		"opid", info.OperationID(),
...
func (c *LongConnMgr) doWsGroupNotificationPrune(ctx context.Context, wsResp GeneralWsResp) error {
	var value sdkws.GroupNotificationPrune
	if err := proto.Unmarshal(wsResp.Data, &value); err != nil {
		return err
	}
	return common.DispatchGroupNotificationPrune(ctx, &value, c.groupMgrCh)
}

func (c *LongConnMgr) NetworkStatusChanged(ctx context.Context) {
	diagnose.Info("net.change",
		"user_id", c.userID,
		"opid", ccontext.Info(ctx).OperationID(),
	)
	c.Close(ctx)
	select {
	case c.netChanged <- struct{}{}:
		log.ZDebug(ctx, "network status changed")
	default:
	}
}
