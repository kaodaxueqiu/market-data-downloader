...
func (c *Conversation) HideConversation(ctx context.Context, conversationID string) error {
	println("[GO-DEBUG] HideConversation called, conversationID=", conversationID)
	...
	println("[GO-DEBUG] HideConversation ResetConversation OK, calling UpdateConversation...")
	req := &pbConversation.UpdateConversationReq{
		ConversationID: conversationID,
		UserIDs:        []string{c.loginUserID},
		IsHidden:       &wrapperspb.BoolValue{Value: true},
	}
	if err := api.UpdateConversation.Execute(ctx, req); err != nil {
		println("[GO-DEBUG] HideConversation UpdateConversation FAILED:", err.Error())
		return err
	}
	println("[GO-DEBUG] HideConversation UpdateConversation SUCCESS")
	return nil
}

func (c *Conversation) UnhideConversation(ctx context.Context, conversationID string) error {
	println("[GO-DEBUG] UnhideConversation called, conversationID=", conversationID)
	_ = c.db.UpdateColumnsConversation(ctx, conversationID, map[string]interface{}{"is_hidden": false})
	req := &pbConversation.UpdateConversationReq{
		ConversationID: conversationID,
		UserIDs:        []string{c.loginUserID},
		IsHidden:       &wrapperspb.BoolValue{Value: false},
	}
	err := api.UpdateConversation.Execute(ctx, req)
	if err != nil {
		println("[GO-DEBUG] UnhideConversation UpdateConversation FAILED:", err.Error())
		return err
	}
	println("[GO-DEBUG] UnhideConversation UpdateConversation SUCCESS")
	return nil
}
