...
var CmdPriorityMap = map[string]int{
	constant.CmdNewMsgCome:         4,
	constant.CmdUpdateConversation: 3,
...
func DispatchGroupNotification(ctx context.Context, msg *sdkws.MsgData, groupCh chan Cmd2Value) error {
	return sendCmdToChan(groupCh, Cmd2Value{Value: msg, Ctx: ctx}, timeout)
}

func DispatchRelationNotification(ctx context.Context, msg *sdkws.MsgData, relationCh chan Cmd2Value) error {
	return sendCmdToChan(relationCh, Cmd2Value{Value: msg, Ctx: ctx}, timeout)
}

func DispatchUserNotification(ctx context.Context, msg *sdkws.MsgData, userCh chan Cmd2Value) error {
	return sendCmdToChan(userCh, Cmd2Value{Value: msg, Ctx: ctx}, timeout)
}

func DispatchGroupNotificationPrune(ctx context.Context, value *sdkws.GroupNotificationPrune, ch chan Cmd2Value) error {
	return sendCmdToChan(ch, Cmd2Value{Cmd: constant.CmdGroupNotificationPrune, Value: value, Ctx: ctx}, timeout)
}
...
type GroupHasReadMessageInfo struct {
	ConversationID string
	ClientMsgID    string
	AttacheInfo    *sdk_struct.AttachedInfoElem
}
type ModifyMessageInfo struct {
	ConversationID string
	ClientMsgID    string
	NewMessage     *sdk_struct.MsgStruct
}
...
// 0 is GetCaller itself,1 is its caller, and so on.
