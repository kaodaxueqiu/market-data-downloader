...
func InitSDK(listener open_im_sdk_callback.OnConnListener, operationID string, config string) (ok bool) {
	diagnose.Info("sdk.init.step2",
		"platform_id", operationID, "config", config)
	...
	fmt.Println("[DLL-DEBUG] InitSDK dataDir=", configArgs.DataDir, " logFilePath=", configArgs.LogFilePath)
	if err := SetHttpConfig(configArgs); err != nil {
		fmt.Println(operationID, "SetHttpConfig failed ", err.Error())
		return false
	}
	if configArgs.DiagnoseHashSalt != "" {
		diagnose.SetHashSalt([]byte(configArgs.DiagnoseHashSalt))
	}
...
func (u *UserContext) NetworkStatusChanged(ctx context.Context) {
	u.longConnMgr.NetworkStatusChanged(ctx)
}
